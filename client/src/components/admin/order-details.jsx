...
import { updateOrderStatusFormControls } from "@/config";
import { updateOrderStatusAdmin } from "@/store/admin/orders-slice";
...
const AdminOrderDetailsView = ({ orderDetails }) => {
  ...
  const handleUpdateStatus = (e) => {
    ...
    dispatch(
      updateOrderStatusAdmin({ id: orderDetails?._id, orderStatus: status })
    )
      .then((data) => {
      ...
      .catch((e) => {
        toast({
          title: e?.response?.data?.message || "Failed to update order status",
          variant: "destructive",
        });
      });
  };
  ...
  return (
    <DialogContent className="sm:max-w-[600px] overflow-y-scroll max-h-screen">
      ...
      <div>
        <CommonForm
          formControls={updateOrderStatusFormControls}
          formData={formData}
